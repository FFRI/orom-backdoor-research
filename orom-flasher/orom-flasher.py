...
def handle_arguments():
    ...
    parser.add_argument(
            '--pagesize', '-s',
            type = lambda x: int(x,0),
            help = 'page size of the flash chip'
            )
    parser.add_argument(
            '--file','-f',
            help = 'orom image file to write'
            )
...
def main():
    ...
    if port!=None:
        ...
        # write orom image
        pageN = (len(image)//PAGESIZE) + 1
        print("OROM page size:",pageN,"filesize:",len(image))
        print("Writing OROM image:", end=" ")
        for i in range(pageN):
            erase(port, ADDR + (i*PAGESIZE))
            if i==(pageN-1):
                fastwrite(port, ADDR+(i*PAGESIZE), image[i*PAGESIZE:(i+1)*PAGESIZE], len(image)-(PAGESIZE*i));
            else:
                fastwrite(port, ADDR+(i*PAGESIZE), image[i*PAGESIZE:(i+1)*PAGESIZE], PAGESIZE);
        print("done");
        ...
        print("check data:")
        read(port, ADDR+0x34, 0x10)

        print("check too much erase:")
        read(port, ADDR+len(image)-0x20, 0x40)
